...
vulnerabilities <- paste(numVulnerabilidades, "vuln.", sep=" ")
```
A continuación, se muestran las vulnerabilidades de los últimos 7 días.
...
#Primer dia
for (i in 1:nrow(exploitsDB)){firstday[i]<-nrow(SoCoTools::twitter_query(words[i],30L,date1[i],date1[i]))}

#Segundo dia
for (i in 1:nrow(exploitsDB)){secondday[i]<-nrow(SoCoTools::twitter_query(words[i],30L,date1[i]+1,date1[i]+1))}

#Tercer dia
for (i in 1:nrow(exploitsDB)){thirdday[i]<-nrow(SoCoTools::twitter_query(words[i],30L,date1[i]+2,date1[i]+2))}
